/*
*	Class Teapot Author: Benjamin Robbins
*	Parent class: Renderer
*	Function: Creates a B-Spline teapot in memory from Patch objects
*	Dependencies: glm, Renderer, Patch, requires vertex and fragment shaders
*/

#include "teapot.h"
#include "patch.h"
#pragma region Control Points
static const GLfloat CONTROL_POINTS[] = {
...
Teapot::Teapot(int numTeapots)
	:Renderer(numTeapots)
{
	_numTeapots = numTeapots;
}
Teapot::~Teapot()
{
...
//Functions
void Teapot::init(const char* fshader, const char* vshader, GLfloat* normalTex, int widthN, int heightN, GLfloat* collideTex, int widthC, int heightC)
{
	Renderer::init(fshader, vshader, normalTex, widthN, heightN, collideTex, widthC, heightC);

	//Create all the patches for the teapot
	for(int i = 0; i < NUM_PATCHES; i++)
	{
		_patches[i] = new Patch();
		_patches[i]->init(fshader, vshader, normalTex, widthN, heightN, collideTex, widthC, heightC);

		//Set the CP positions of the patch
		
		for(int j = 0; j < 37; j += 12)
		{
			(*(_patches)[i])[0 + j/3] = glm::vec3(CONTROL_POINTS[j + i*48], CONTROL_POINTS[j + 1 + i*48], CONTROL_POINTS[j + 2 + i*48]);
			(*(_patches)[i])[1 + j/3] = glm::vec3(CONTROL_POINTS[j + 3 + i*48], CONTROL_POINTS[j + 4 + i*48], CONTROL_POINTS[j + 5 + i*48]);
			(*(_patches)[i])[2 + j/3] = glm::vec3(CONTROL_POINTS[j + 6 + i*48], CONTROL_POINTS[j + 7 + i*48], CONTROL_POINTS[j + 8 + i*48]);
			(*(_patches)[i])[3 + j/3] = glm::vec3(CONTROL_POINTS[j + 9 + i*48], CONTROL_POINTS[j + 10 + i*48], CONTROL_POINTS[j + 11 + i*48]);
		}

		//Apply control point changes to verts
		_patches[i]->berp();
	}
}

void Teapot::display(GLint teapot)
{
	Renderer::display(teapot);
	for(int i = 0; i < NUM_PATCHES; i++)
	{
		_patches[i]->transform(0) = _transforms[teapot];
		_patches[i]->update(_time);
		_patches[i]->display();
	}
}

void Teapot::update(GLfloat time)
{
	_time = time;
